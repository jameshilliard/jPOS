    ...
    */
    ...
    public String getCountersAsString () {
        ...
        if (!lastTxn.equals(Instant.EPOCH)) {
            sb.append (", idle=");
            sb.append(Duration.between(lastTxn, Instant.now(NanoClock.systemUTC())).toMillis());
            sb.append ("ms");
        }
        return sb.toString();
    }
    ...
    public long getIdleTimeInMillis() {
        return !lastTxn.equals(Instant.EPOCH) ? Duration.between(lastTxn, Instant.now(NanoClock.systemUTC())).toMillis() : -1L;
    }
