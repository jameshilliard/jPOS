...
public class MUXPool extends QBeanSupport implements MUX, MUXPoolMBean {
    ...
    public ISOMsg request (ISOMsg m, long timeout) throws ISOException {
        ...
        if (mux != null) {
            long duration = Duration.between(now, Instant.now(NanoClock.systemUTC())).toMillis();
            if (timeout >= duration)
        ...
        do {
            for (MUX m : mux)
                if (isUsable(m))
                    return m;
            ISOUtil.sleep (1000);
        } while (Duration.between(now, Instant.now(NanoClock.systemUTC())).toMillis() < maxWait);
        return null;
    }
