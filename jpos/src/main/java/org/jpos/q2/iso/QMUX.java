    ...
    implements SpaceListener, MUX, QMUXMBean, Loggeable, MetricsProvider
{
    ...
    public ISOMsg request (ISOMsg m, long timeout) throws ISOException {
        ...
        try {
            ...
            synchronized (this) {
                ...
                {
                    rx++;
                    lastTxn = Instant.now(NanoClock.systemUTC()).toEpochMilli();
                }else {
                    rxExpired++;
    ...
    public String getCountersAsString () {
        ...
        if (lastTxn > 0) {
            sb.append (", idle=");
            sb.append(Instant.now(NanoClock.systemUTC()).toEpochMilli() - lastTxn);
            sb.append ("ms");
        }
        return sb.toString();
    }
    ...
    public long getIdleTimeInMillis() {
        return lastTxn > 0L ? Instant.now(NanoClock.systemUTC()).toEpochMilli() - lastTxn : -1L;
    }
