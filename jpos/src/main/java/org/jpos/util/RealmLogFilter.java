...
public class RealmLogFilter implements LogListener, XmlConfigurable, Configurable {
    ...
    private Instant lastDump = Instant.now(NanoClock.systemUTC());
    ...
    public LogEvent log(LogEvent ev) {
        ...
        if (realmEnabled(realm)) {
            ...
            if (dumpInterval > 0 && Duration.between(lastDump, Instant.now(NanoClock.systemUTC())).toMillis() > dumpInterval) {
                LogEvent evt = new LogEvent("ignored-realms");
                evt.addMessage(realmsMissed);
                realmsMissed = new HashSet<>();
                lastDump = Instant.now(NanoClock.systemUTC());
                return evt;
            }
            return null;
        }
